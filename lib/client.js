...
MqttClient.prototype._handlePacket = function (packet, done) {
  ...
  // Consider all responses as ping responses if we are doing that
  if (this.options.reschedulePings){
  ...
  switch (packet.cmd) {
    ...
    case 'connack':
      ...
      break
    case 'pingresp':
      //Ping responses are ping responses if everything is not a ping response
      if (!this.options.reschedulePings){
        this._handlePingresp(packet)
      }
      done()
...
MqttClient.prototype._setupPingTimer = function () {
  ...
  if (!this.pingTimeoutTimer && this.options.connectionIdleTimeout) {
    this.pingRespLast = new Date()
    this.pingTimeoutTimer = reInterval(function() {
      that._checkPingTimeout()
...
MqttClient.prototype._shiftPingInterval = function () {
  if (this.pingTimer && this.options.keepalive && this.options.reschedulePings) {
    this.pingTimer.reschedule(this.options.connectionIdleTimeout * 1000)
  }
}
/**
...
/**
 * _checkPingTimeout - check if a pingresp has come back at all, otherwise kill the connection
 *
 * @api private
 */
MqttClient.prototype._checkPingTimeout = function () {
  var now = new Date()
  var elapsedMs = now.getTime() - this.pingRespLast.getTime()
  var thresholdMs = this.options.connectionIdleTimeout * 1000
